...
class AddNewBookUseCase(
    ...
    override fun map(action: AddBookAction, state: AddBookState): AddBookResult {
        return try {
            if (state.bookName.isNotEmpty() && state.bookDescriptions.isNotEmpty() && state.authors.isNotEmpty()) {
                ...
                AddBookResult.SaveNewBookError(
                    titleIsEmpty = state.bookName.isEmpty(),
                    descriptionsIsEmpty = state.bookDescriptions.isEmpty(),
                    authorsIsEmpty = state.authors.isEmpty()
                )
            }
        } catch (e: Exception) {
